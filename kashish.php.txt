...
if ($_SERVER["REQUEST_METHOD"]=="POST"){
	//Check if a file was uploaded without any error or not
	if(isset($_FILES["file"]) && $_FILES["file"]["error"]==0){
		...
		if (!in_array($file_type, $allowed_types)){
			...
			if (move_uploaded_file($_FILES["file"]["tmp_name"],$target_file)){
				//File uplod sucessfully ,now store information in the database
              ...
              $sql ="INSERT INTO files (filename,filesize,filetype) VALUES ('$filename',$filesize,'$filetype')";
                 ...
                 if ($conn->query($sql) === TRUE) {
                    ...
                    echo "Sorry,there was an error uploading your file and storing  information in thedatabase:" .$conn->error;
                }     
